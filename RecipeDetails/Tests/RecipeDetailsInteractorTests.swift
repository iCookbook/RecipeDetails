...
class RecipeDetailsInteractorTests: XCTestCase {
    ...
    var presenter: SpyRecipeDetailsPresenter!
    /// SUT.
    var interactor: RecipeDetailsInteractor!
    
    override func setUpWithError() throws {
        interactor = RecipeDetailsInteractor()
        presenter = SpyRecipeDetailsPresenter()
    ...
    func testProvideData() throws {
        interactor.provideData()
        
        XCTAssertNotNil(presenter.recipe)
        XCTAssertNotNil(presenter.isFavourite)
    }
    
    func testAddRecipeToFavourites() throws {
        let oldCount = UserDefaults.favouriteRecipes.count
        
        interactor.addRecipeToFavourites()
        
        let newCount = UserDefaults.favouriteRecipes.count
        XCTAssertNotEqual(oldCount, newCount)
        XCTAssert(newCount > oldCount)
    }
    
    func testRemoveRecipeFromFavourites() throws {
        let oldCount = UserDefaults.favouriteRecipes.count
    ...
    func testProvideRecipeSource() throws {
        interactor.provideRecipeSource()
        
        XCTAssertNotNil(presenter.urlString)
    }
}
